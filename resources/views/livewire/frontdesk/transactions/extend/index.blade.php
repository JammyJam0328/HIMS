<div x-data="{ showExtendForm: false }"
    x-on:close-form.window="showExtendForm=false">
    <x-content>
        ...
        <div wire:key="guest">
            @if ($guest)
                <div id="9091873ashdjashduiy128372891">
                    <div x-show="!showExtendForm"
                        id="view"
                        class="grid w-full gap-5">
                        <div class="flex justify-end col-span-12">
                            <x-button.primary x-on:click="showExtendForm=true">
                                Extend Guest
                            </x-button.primary>
                        </div>
                    ...
                    <div x-cloak
                        ...
                        <div x-data="{ excessAmountFromPreviousPayment: $wire.entangle('excessAmountFromPreviousPayment') }"
                                ...
                                <div class="grid items-start grid-cols-2 gap-4">
                                    <div class="grid col-span-1 gap-4">
                                        ...
                                        <div class="grid gap-1">
                                            <x-input-label value="Select Hours" />
                                            <x-select wire:model="extensionId">
                                                <option wire:key="qidjashduq82qdqw"
                                                    hidden
                                                    value="">
                                                    Select Type
                                                </option>
                                                @foreach ($extensionRates as $extensionRate)
                                                    <option value="{{ $extensionRate->id }}">
                                                        {{ $extensionRate->hour }} hrs
                                                    </option>
                                                @endforeach
                                            </x-select>
                                        </div>
                            ...
                            <div class="mt-5 flex justify-end space-x-3 rounded-b-lg border-t bg-white p-2.5">
                                <x-button.danger x-on:click="showExtendForm = false">Close Form</x-button.danger>
                                <x-button.primary x-on:click.prevent="$dispatch('confirm-extend')">
                                    Save
                                </x-button.primary>
                            </div>
                        </div>
