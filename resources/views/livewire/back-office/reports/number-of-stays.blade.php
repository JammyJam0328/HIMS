<div x-data>
  <div class="flex items-center justify-between">
    ...
    <div class="flex space-x-1">
      ...
      <x-button.primary @click="printOut($refs.printContainer.outerHTML);">
        Print
      </x-button.primary>
    </div>
  </div>
  <div class="p-5 mt-5 border-2">
    <div x-ref="printContainer" id="print" class="flex flex-col">
      ...
      <table id="example" class="mt-5 table-auto" style="width:100%">
        <thead class="font-normal">
          <tr>
            <th class="px-2 py-2 text-sm font-semibold text-left text-gray-700 border border-gray-700">ROOM
              #
            </th>
            <th class="px-2 py-2 text-sm font-semibold text-left text-gray-700 border border-gray-700">TYPE
              OF
              ROOM</th>
            <th class="px-2 py-2 text-sm font-semibold text-left text-gray-700 border border-gray-700">RATE
            </th>
            <th class="px-2 py-2 text-sm font-semibold text-left text-gray-700 border border-gray-700">GUEST
              NAME</th>
          ...
          @foreach ($guests as $guest)
            <tr>
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                {{ $guest->room->numberWithFormat() }}
              </td>
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                {{ $guest->room->type->name }}
              </td>
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                &#8369; {{ number_format($guest->transactions->first()->payable_amount, 2) }}
              </td>
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                {{ $guest->name }}
              </td>
              ...
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                {{ \Carbon\Carbon::parse($guest->checked_out_at)->format('M d Y h:i A') }}
              </td>
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                @if ($guest->transactions_sum_payable_amount)
                  &#8369; {{ number_format($guest->transactions_sum_payable_amount, 2) }}
                @else
                  No Damages
                @endif
              </td>
              <td class="px-2 py-2 text-sm text-gray-700 border border-gray-700">
                @if ($guest->deposits_sum_amount)
                  &#8369; {{ number_format($guest->deposits_sum_amount, 2) }}
                @else
                  No Deposits
                @endif
              </td>
