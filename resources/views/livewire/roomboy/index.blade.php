{{-- <div x-data>
  @if (auth()->user()->room_boy_assigned_floor_id)
    ...
    <x-confirm name="finish-cleaning" title="Confirm" message="Are you sure you want to finish cleaning this room?"
...
<div x-data>
    ...
    <div class="flex items-center space-x-5">
      ...
      <div>
        ...
        <p class="text-sm font-medium text-gray-400 flex  items-center">
          ...
          <span class="ml-1"> {{ auth()->user()->roomBoyFloor->numberWithFormat() }}</span>
        </p>
      </div>
    </div>
    <div
  ...
  <div class="grid max-w-3xl grid-cols-1 gap-6 mx-auto mt-5 sm:px-6 lg:max-w-7xl lg:grid-flow-col-dense lg:grid-cols-3">
            ...
            <div class="">
                    ...
                    <div
                      ...
                      <div class="px-7">
                        <h1 class="font-bold text-3xl text-gray-600">
                          {{ auth()->user()->roomBoyRoom->numberWithFormat() }}</h1>

                        <div class="flex space-x-3 mt-2">
                          <span>Time Started:</span>
                          <span>{{ \Carbon\Carbon::parse(auth()->user()->roomBoyRoom->started_cleaning_at)->diffForHumans() }}</span>
                        </div>
                        <div class="mt-1">
                          <x-button.dark x-on:click="$dispatch('confirm-finish-cleaning')">
                            Finish Cleaning
                          </x-button.dark>
                        </div>
                      </div>
              ...
              <div class="px-4 mt-4 xl:px-2">
                ...
                <div class="grid xl:grid-cols-2 grid-cols-1 py-1 gap-3">
                  @forelse ($assignedRooms as $assignedRoom)
                    <div
                      ...
                      <div class="px-7">
                        <h1 class="font-bold text-3xl text-gray-600"> {{ $assignedRoom->numberWithFormat() }}</h1>
                        @php
                          $expires = \Carbon\Carbon::parse($assignedRoom->time_to_clean);
                        @endphp
                        <div class="flex space-x-3 mt-2">
                ...
                <div class="grid xl:grid-cols-2 grid-cols-1  gap-3 py-1">
                  @forelse ($unassignedRooms as $unassignedRoom)
                    <div
                      ...
                      <div class="px-7">
                        <h1 class="font-bold text-3xl text-gray-600"> {{ $unassignedRoom->numberWithFormat() }}</h1>
                        @php
                          $expires = \Carbon\Carbon::parse($unassignedRoom->time_to_clean);
                        @endphp
                        <div class="flex space-x-3 mt-2">
