@props(['name', 'title', 'message', 'confirmButtonText' => 'Confirm', 'cancelButtonText' => 'Cancel', 'params' => [], 'onConfirm' => null, 'onCancel' => null])

<div id="{{ $name }}"
    x-data="{ show: false }"
    x-on:confirm-{{ $name }}.window="show = true; params = $event.detail">
    <div x-cloak
        x-show="show"
        class="relative z-10"
            ...
            <div class="flex items-end justify-center min-h-full p-4 text-center sm:items-center sm:p-0">
                ...
                <div x-cloak
                    ...
                    <div class="px-4 py-3 bg-gray-50 sm:flex sm:flex-row-reverse sm:px-6">
                        <button type="button"
                            @if ($onConfirm) x-on:click="$wire.{{ $onConfirm }}; show = false"
                            @else
                            x-on:click="show = false" @endif
                        ...
                        <button type="button"
                            @if ($onCancel) x-on:click="$wire.{{ $onCancel }}; show = false"
                            @else
                            x-on:click="show = false" @endif
